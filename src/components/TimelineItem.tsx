  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center shrink-0">
        {/* Dot */}
        <div
          className={`w-3 h-3 rounded-full mt-1 z-10 shrink-0 ${
            isMandatory
      ...
      >
        ...
        <div className="flex items-start justify-between gap-2 mb-1">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-gold font-mono text-sm font-semibold tabular-nums">
              {item.time}
            </span>
            {isMandatory && (
              <span className="text-[10px] px-1.5 py-0.5 rounded bg-gold/20 text-gold font-medium leading-none">
                確定
              </span>
            )}
            {isHighlight && !isMandatory && (
              <Star size={12} className="text-gold/60" />
            )}
          </div>
          {/* Transport badge */}
          {item.transport !== "none" && (
            <div
              className={`flex items-center gap-1 text-[11px] shrink-0 ${transport.color}`}
            >
        ...
        {item.link && (
          <a
            ...
            className="mt-2 inline-flex items-center gap-1 text-gold/70 text-xs active:opacity-70"
          >
